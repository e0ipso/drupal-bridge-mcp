...
## Objective
Implement the OAuth 2.0 Authorization Code Grant flow manager for secure integration with Drupalize.me's Simple OAuth module, including state validation and token exchange.
...
Follow the OAuth 2.0 RFC 6749 specification exactly. The Simple OAuth module in Drupal expects standard OAuth parameters. Use URLSearchParams for proper parameter encoding.

Implement comprehensive error handling for network failures, invalid responses, and authorization denials. The state parameter should be stored temporarily and cleaned up after successful validation.

Design the token exchange to handle both success and error responses from the Drupal OAuth endpoint, with proper parsing of the JSON response structure.