...
## Objective
Implement health check endpoints, application performance monitoring, database connection monitoring, and OAuth token refresh tracking for operational visibility.
...
Keep health checks lightweight to avoid impacting performance. Include database connection pooling status. Track OAuth token refresh failures as critical metric. Use Railway's built-in monitoring where possible to minimize external dependencies. Structure logs for easy parsing and alerting.