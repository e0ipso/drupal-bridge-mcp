...
**Problem**: Simple OAuth designed for stateless HTTP requests, but MCP often uses long-running connections (stdio/SSE)
**Impact**: Token validation per-request vs per-connection authentication
**Solutions**:
...
**Problem**: OAuth tokens expire (typically 1-3600 seconds), breaking MCP sessions
**Impact**: Connection drops, tool discovery failures, interrupted workflows
**Solutions**:
...
**Problem**: MCP server needs granular access to multiple Drupal subsystems
**Required Scopes**:
...
**Problem**: RAG system may need different user contexts for content access
**Challenges**:
...
**Impact**: Discovery failures, search timeouts, content retrieval bottlenecks
**Solutions**:
