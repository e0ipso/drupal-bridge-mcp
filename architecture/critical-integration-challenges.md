...
This document outlines the most critical technical challenges in integrating Drupal's Simple OAuth
and JSON-RPC modules with a Model Context Protocol (MCP) server for RAG system enhancement. Each
challenge is analyzed with specific solutions based on the architectural decisions made.
...
**OAuth 2.0 is designed for stateless HTTP requests**, but MCP servers often maintain **long-running
connections** (stdio, SSE). Token expiration can break active sessions.
...
**JSON-RPC methods are static** (defined by installed Drupal modules), but **MCP tools must be
dynamic** (discovered at runtime and updated when methods change).
...
**Content transformation complexity eliminated** by moving transformation to Drupal-side using
JSON-RPC methods that return pre-formatted Markdown.
...
**Errors can occur at multiple layers** (OAuth, JSON-RPC, Drupal, Solr, MCP protocol) and **must be
translated meaningfully** for the connected LLM and end user.
...
**Content access varies by user subscription levels** (free vs. subscriber), and **each user must
...
This architecture addresses the critical integration challenges while maintaining simplicity,
