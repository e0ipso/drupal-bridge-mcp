...
The initial architecture considered having the MCP server make its own connections to LLM providers (OpenAI, Anthropic, etc.) for query parsing and content processing. However, this introduces several complications:
...
The user explicitly stated: *"I don't want the mcp server to require an LLM key to connect. Since the mcp is going to be interacting with an LLM already, I would like it to just ask back the questions that it needs instead of creating a new connection to an LLM provider."*

## Decision

The MCP server will NOT maintain its own LLM connections. Instead, it will leverage the existing LLM client connected via the MCP protocol for any intelligent processing needs.
...
**Description**: MCP server maintains own LLM connections
**Rejected Because**: 
...
**Description**: Optional LLM integration for advanced features
**Rejected Because**:
...
**Description**: Pre-process content with offline LLM processing
**Rejected Because**:
